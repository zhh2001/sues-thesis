...
常用色系有三原色 rgb 和灰度 gray 两种；
被预定义的色名有68个，目前貌似被废弃，所以下面我使用了\verb|definecolor|定义颜色，可以用RGB颜色定义也可以用HTML十六进制格式定义，详见 color 宏包介绍中所附的色标；左右伸出的长度单位可用 pt。
...
\subsection{普通插图方法}
如\autoref{fig:my_pic1}所示，在这里我们插入了一个图片。
其中可以自定义缩放比例以及图片的高度等等信息。
\begin{figure}[hbpt]
    \centering
...
\section{画表和图出现的问题}
在提交的院审和校审中出现有同学，插图位置不当、表格插入位置不当的问题。这里提以下的一些意见：
\begin{enumerate}[label=(\arabic*)]
    \item 一般地，行文时，根据插图在论文中被提及的顺序，插图应安排在第一次提及它的段落后面，不要放在一段话的中间。
    \item 图应具有自明性，一般随文编排，先文字后见图，先出现图后面才有表述。
    \item 表格插入位置不当，表格一般应随文列出，紧接在第一次涉及它的文字段后面。
    \item 注意灵活应用h b p t 以及H的使用，尽量每个页面都有图文混排的方式，如果部分页面下部留白太多，要把下页的文字移上来，或者缩小图表调整一下，灵活调整表格和图片的位置。
\end{enumerate}
