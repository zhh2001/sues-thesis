...
另外，关于Legendre符号有以下的定理
\begin{theorem}[Legendre符号]
    Legendre符号有以下的性质
...
\begin{proof}[证明\cref{lem:example}]
    ...
    \end{equation}
    根据\cref{thm:Legendre}和\cref{thm:LegendreExtension}可以得到
    \begin{equation}
...
\begin{conjecture}[霍奇猜想]
    给定一个随机形状，它可以和一个多项式描述的形状同胚。
    换句话说，我们能够取任意曲面并通过适当变形操作，最终得到一个多项式的解集
    \label{con:example}
\end{conjecture}
